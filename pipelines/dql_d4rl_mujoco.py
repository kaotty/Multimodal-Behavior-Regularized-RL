...
def svgd_update(a_0, s, itr_num, svgd_step, batch_size, num_particles, act_dim, alpha, critic, kernel, device):
    ...
    for l in range(itr_num):
        ...
        h = (K_value.matmul(score_func)/alpha) / num_particles # - K_grad.sum(2)) / num_particles
        ...
        k_value = K_value.abs().sum(-1).mean(-1)
        ...
        tr = tr + svgd_step * term1 #(term1 + term2)
        a = a + svgd_step * h
    ...
    return a, tr, score, a_0_cv, a_svgd_cv, q_grad, k_grad, k_value
...
def pipeline(args):
    ...
    wandb.init(project="dql_mujoco", 
               ...
               name=f"without-trace-{args.task.env_name}-itr_num:{args.itr_num}-alpha:{args.alpha}-epsilon:{args.svgd_step}-seed:{args.seed}")

    logger = logging.getLogger('my_logger')
    logger.setLevel(logging.INFO)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    os.makedirs(os.path.join(script_dir,'logs'), exist_ok=True)
    log_path = os.path.join(script_dir, 'logs', 'without-trace-no_k_grad-{}-itr{}-alpha{}'.format(args.task.env_name,args.itr_num,args.alpha))
    ...
    actor.eval()
    log = {"critic_loss": 0., "target_q_mean": 0., "trace": 0., "score_max": 0., "score_min":0., "score_mean":0., "a_0_cv": 0., "a_svgd_cv": 0., "q_grad": 0., "k_grad": 0., "k_value": 0.}
    ...
    for batch in loop_dataloader(dataloader): 
        ...
        next_act, tr, score, a_0_cv, a_svgd_cv, q_grad, k_grad, k_value = svgd_update(a_0, next_obs, args.itr_num, args.svgd_step, args.batch_size, args.training_num_particles, act_dim, args.alpha, critic, kernel, args.device)
        # mmd = mmd_linear(a_0, next_act)
        
        target_q_1, target_q_2 = critic_target(next_obs, next_act)
        target_q = torch.min(target_q_1, target_q_2)# + args.alpha * tr # discard trace
        target_q = target_q.view(-1, args.training_num_particles, 1).max(1).values # max
        ...
        if (svgd_gradient_step + 1) % args.log_interval == 0:
            ...
            logger.info("a_0_cv: {}, a_svgd_cv: {}".format(log["a_0_cv"], log["a_svgd_cv"]))
            logger.info("q_grad: {}, k_grad: {}, k_value: {}".format(log["q_grad"], log["k_grad"], log["k_value"]))
            log = {"critic_loss": 0., "target_q_mean": 0., "trace": 0., "score_max": 0., "score_min":0., "score_mean":0., "a_0_cv": 0., "a_svgd_cv": 0., "q_grad": 0., "k_grad": 0., "k_value": 0.}
        ...
        if (svgd_gradient_step + 1) % args.inference_interval == 0:
            ...
            for i in range(args.num_episodes):
                ...
                while not np.all(cum_done) and t < 1000 + 1:
                    ...
                    new_act, tr, score, a_0_cv, a_svgd_cv, q_grad, k_grad, k_value = svgd_update(act, obs, args.itr_num, args.svgd_step, args.num_envs, args.num_candidates, act_dim, args.alpha, critic, kernel, args.device)

                    # resample
                    with torch.no_grad():
                        q = critic_target.q_min(obs, new_act)
                        q = q.view(-1, args.num_candidates, 1)
                        w = torch.softmax(q * args.task.weight_temperature, 1) # add max q
                    ...
                    # print(sampled_act.size(), obs.size())
